...
'Read comments at bottom of module for implementation tips
...
Private Function MenuProc(ByVal hwnd As Long, ByVal wMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
    ...
    Select Case wMsg&
        Case WM_MEASUREITEM
            ...
            MeasureInfo.ItemWidth = ParForm.TextWidth(BB.Caption) + 36 + IIf(Len(BB.KeyAccel) = 0, 0, ParForm.TextWidth(BB.KeyAccel) + 4)
        ...
        Case WM_DRAWITEM
            ...
            If Val(BB.vbMenuID.Tag) > 0 Then
                If IsChecked Then
                    ...
                    SetRect chkR, 2, R.Top, 23, R.Top + 20
...
'    mnuFileAssociate.Tag = 8
'    mnuFileAssocRTF.Tag = 9
'    mnuEdit(0).Tag = 10
'    mnuEdit(2).Tag = 11
'    mnuEdit(3).Tag = 12
'    mnuEdit(4).Tag = 13
...
'    ConvertOD Me
'End Sub
'To load a menu at runtime...
'*mnuTest is an existing array of menus
'**mnuTestbase is the submenu containing mnuTest menus
'    Load mnuTest(mnuTest.Count)
'    mnuTest(mnuTest.Count - 1).Tag = 16 'set the icon image
'    mnuTest(mnuTest.Count - 1).Caption = "blah" ' set the caption
'    AddODMenu mnuTest(mnuTest.Count - 1), mnuTestbase ' convert to ownerdrawn
'
'
'***** REQUIRES CLASS MODULE *******
'Class BBMenu - not much to this class...
'
'Option Explicit
...
'I've used the menus' tag property to store the image index - you
'could store this in a collection(for example) if you need to use
'the tag property

'You dont need to use an imagelist, you could paint from any source

'If you apply this to an MDI form you will need to allow
'for the fact that MDI scalemode cannot be set to Pixels
'so you will have to adjust numbers for painting icons
'and change declarations of "Form" to "Object".
